...
ggplot(aes(x=deaths_per_100k, y=fully_vaccinated_pct_of_pop, colour="red", label=name))+
  geom_point() +geom_text(hjust=0.2, vjust=0) + geom_smooth(method = lm) +ggtitle("Covid-19 deaths since universal adult vaccine eligibility compared with vaccination rates") +
  theme(panel.background = element_rect(fill = "skyblue", color = "red")) 
